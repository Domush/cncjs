    ...
    this.feeder = new Feeder({
      dataFilter: (line, context) => {
...

        {
          ...
          if (programMode === 'M0') {
            log.debug(`M0 Program Pause: line=${line}, sent=${context}`);
            ...
            log.debug(`M1 Program Pause: line=${line}, sent=${context}`);
  ...
  command(cmd, ...args) {
    const handler = {
      ...
      'laser:on': () => {
        ...
        const commands = ['M3S' + ensurePositiveNumber(maxS * (power / 100))];
